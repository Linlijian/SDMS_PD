...
namespace FluentValidation
{
    ...
    public static class DefaultValidatorExtensions
    {
        ...
        /// <summary>
        /// ActiveAndStatusMatch Validate Active And Status Not Mated
        /// </summary>
        /// <typeparam name="T">The type of object being validated</typeparam>
        /// <typeparam name="TProperty">Type of property being validated</typeparam>
        /// <param name="ruleBuilder">The rule builder on which the validator should be defined</param>
        /// <param name="expression">A lambda expression to provide the comparison value</param>
        /// <returns></returns>
        public static IRuleBuilderOptions<T, TProperty> ActiveAndStatusMatch<T, TProperty>(this IRuleBuilder<T, TProperty> ruleBuilder, Expression<Func<T, TProperty>> expression)
        {
            var func = expression.Compile();
            return ruleBuilder.SetValidator(new ActiveAndStatusMatchValidator(func.CoerceToNonGeneric(), expression.GetMember()));
        }
        ...
        public static IRuleBuilderOptions<T, TProperty> IDCard<T, TProperty>(this IRuleBuilder<T, TProperty> ruleBuilder, Expression<Func<T, TProperty>> expression)
        {
            var func = expression.Compile();
            return ruleBuilder.SetValidator(new IDCardValidator(func.CoerceToNonGeneric(), expression.GetMember()));
        }
        ...
        public static IRuleBuilderOptions<T, TProperty> Store<T, TProperty>(this IRuleBuilder<T, TProperty> ruleBuilder, string keyId, params Expression<Func<T, object>>[] expression)
        {
            List<System.Reflection.MemberInfo> members = null;
            List<Func<object, object>> funcs = null;
            if (expression != null)
            {
                funcs = new List<Func<object, object>>();
                members = new List<System.Reflection.MemberInfo>();
                foreach (var item in expression)
                {
                    funcs.Add(item.Compile().CoerceToNonGeneric());
                    members.Add(item.GetMember());
                }
            }
            return ruleBuilder.SetValidator(new StoreValidator(keyId, System.Web.HttpContext.Current.Request.ApplicationPath, funcs, members));
        }

        public static IRuleBuilderOptions<T, string> NotFileExeBat<T>(this IRuleBuilder<T, string> ruleBuilder)
        {
            return ruleBuilder.SetValidator(new NotFileExeBatValidator());
        }

        public static IRuleBuilderOptions<T, string> ChackPassword<T>(this IRuleBuilder<T, string> ruleBuilder)
        {
            return ruleBuilder.SetValidator(new CheckPasswordValidator());
        }
        
        /// <summary>
        /// Defines an 'equalsAfter' validator on the current rule builder. 
        /// Validation will fail if the specified value is not equal to the value of the property.
        /// </summary>
        /// <typeparam name="T">Type of object being validated</typeparam>
        /// <typeparam name="TProperty">Type of property being validated</typeparam>
        ...
        /// <param name="comparer">Equality Comparer to use</param>
        /// <returns></returns>
        public static IRuleBuilderOptions<T, TProperty> EqualAfter<T, TProperty>(this IRuleBuilder<T, TProperty> ruleBuilder, TProperty toCompare, IEqualityComparer comparer = null)
        {
            return ruleBuilder.SetValidator(new EqualAfterValidator(toCompare, comparer));
        }

        /// <summary>
        /// Defines an 'equalsAfter' validator on the current rule builder using a lambda to specify the comparison value.
        /// Validation will fail if the value returned by the lambda is not equal to the value of the property.
        /// </summary>
        /// <typeparam name="T">The type of object being validated</typeparam>
        /// <typeparam name="TProperty">Type of property being validated</typeparam>
        ...
        /// <param name="comparer">Equality comparer to use</param>
        /// <returns></returns>
        public static IRuleBuilderOptions<T, TProperty> EqualAfter<T, TProperty>(this IRuleBuilder<T, TProperty> ruleBuilder, Expression<Func<T, TProperty>> expression, IEqualityComparer comparer = null)
        {
            var func = expression.Compile();
            return ruleBuilder.SetValidator(new EqualAfterValidator(func.CoerceToNonGeneric(), expression.GetMember(), comparer));
        }


        public static IRuleBuilderOptions<T, TProperty> CheckBoxCheck<T, TProperty>(this IRuleBuilder<T, TProperty> ruleBuilder, Expression<Func<T, TProperty>> expression)
        {
            var func = expression.Compile();
            return ruleBuilder.SetValidator(new CheckBoxCheckValidator(func.CoerceToNonGeneric(), expression.GetMember()));
        }

        public static IRuleBuilderOptions<T, TProperty> NotEmptyWithCheckbox<T, TProperty>(this IRuleBuilder<T, TProperty> ruleBuilder, Expression<Func<T, TProperty>> expression)
        {
            var func = expression.Compile();
            return ruleBuilder.SetValidator(new NotEmptyWithCheckboxValidator(func.CoerceToNonGeneric(), expression.GetMember()));
        }
    }
}