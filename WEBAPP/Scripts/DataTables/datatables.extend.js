...
var SARA_E = 'e35';//'111000110101';
var SARA_AI_MAIMALAI = 'e44';//'111001000100';
var MAITAIKHU = 'e47';//'111001000111';
var THANTHAKHAT = 'e46';//'111001000110';
...
function isLeadingVowel(c) {
    return (convertToHex(c) >= SARA_E && convertToHex(c) <= SARA_AI_MAIMALAI);
}
function isToneMark(c) {
    return (convertToHex(c) >= MAITAIKHU && convertToHex(c) <= THANTHAKHAT);
}
...
jQuery.extend(jQuery.fn.dataTableExt.oSort, {
    "date-th-pre": function (a) {
        ...
        if ($.trim(a) !== '') {
            ...
            var m_time = '00';
            var s_time = '00';
            if (t_date != '' && t_date != undefined && t_date != "undefined") {
                h_time = t_date.split(":")[0];
                m_time = t_date.split(":")[1];
                s_time = t_date.split(":")[2];
            }
            var yearVal = d_date[2];

            var myDate = new Date(yearVal, d_date[1] - 1, d_date[0], h_time, m_time, s_time, 000);
    ...
    "date-desc": function (a, b) { // กรณีให้เรียงจากมากไปน้อย
        return b - a;
    },
    ...
    "string-pre": function (a) {
        return _empty(a) ?
            '' :
            typeof a === 'string' ?
                getThaiComparisonString(a.toLowerCase().trim()) :
                !a.toString ?
                    '' :
                    a.toString();
    },
    "string-asc": function (x, y) {
        //var tmp_x = getThaiComparisonString(x.trim().toLowerCase());
        //var tmp_y = getThaiComparisonString(y.trim().toLowerCase());
        x = isNullOrEmpty(x) ? "" : x;
        y = isNullOrEmpty(y) ? "" : y;
        return ((x.trim().toLowerCase() < y.trim().toLowerCase()) ? -1 : ((x.trim().toLowerCase() > y.trim().toLowerCase()) ? 1 : 0));

        //return ((tmp_x < tmp_y) ? -1 : ((tmp_x > tmp_y) ? 1 : 0));
    },
